...
public class PatientSingleton extends ApplicationUser
{
    ...
    private String                   patientSignedHIPAANoticeId;
    private PatientSignedHIPAANotice patientSignedHIPAANotice;
        ...
        try
        {
            ...
            if( jsonObject.has( DB.KEY_USER_EMAIL ) )
            {
                ...
                if( jsonObject.has( DB.KEY_USER_LOGIN_TOKEN )
                    ...
                    patientSingleton.setLoginToken(
                            jsonObject.getString( DB.KEY_USER_LOGIN_TOKEN ) );
                ...
                if( jsonObject.has( DB.KEY_PATIENT_SIGNED_HIPAA_NOTICE_ID )
                        && !jsonObject.getString( DB.KEY_PATIENT_SIGNED_HIPAA_NOTICE_ID )
                        .equals( nullString )
                        && !jsonObject.getString( DB.KEY_PATIENT_SIGNED_HIPAA_NOTICE_ID )
                        .isEmpty() )
    ...
    public static void eraseData()
    {
        ...
        patientSingleton.setPatientSignedHIPAANoticeId( "" );
        patientSingleton.setPatientSignedHIPAANotice( null );
    ...
    public String getPatientSignedHIPAANoticeId()
    {
        return patientSignedHIPAANoticeId;
    }

    public void setPatientSignedHIPAANoticeId( String patientSignedHIPAANoticeId )
    {
        this.patientSignedHIPAANoticeId = patientSignedHIPAANoticeId;
    }

    public PatientSignedHIPAANotice getPatientSignedHIPAANotice()
    {
        return patientSignedHIPAANotice;
    }

    public void setPatientSignedHIPAANotice( PatientSignedHIPAANotice patientSignedHIPAANotice )
    {
        this.patientSignedHIPAANotice = patientSignedHIPAANotice;
    }
...
//                    patient.getPatientSignedHIPAANotice().getRemoteId() );
    ...
    {
        // If we want unsynced and it's not synced...
        return ( ( syncStatus == SyncStatus.UNSYNCED ||
                syncStatus == SyncStatus.ALL )
                && !syncable.isSynced() )
                ||
                // Or, if we want synced and it's synced, return true
