package com.healthmanagement.diabetesassistant.services;
...
import com.healthmanagement.diabetesassistant.adapters.DataSyncAdapter;
...
public class SyncService extends Service
{
	...
	public void onCreate()
	{
		...
		ContentResolver.setSyncAutomatically( mAccount, DAContentProvider.AUTHORITY, true );
		// Turn on periodic syncing:
		ContentResolver.addPeriodicSync(
				mAccount,
				DAContentProvider.AUTHORITY,
				Bundle.EMPTY,
	...
	private void setToSyncWhenTablesChange()
	{
		 ...
		 */
		mResolver.registerContentObserver( DAContentProvider.USERS_URI,
				true, new TableObserver( new Handler() ) );
		// Note: shouldn't have to sync doctors *from* Android
		//mResolver.registerContentObserver( DAContentProvider.DOCTORS_URI,
		//		true, new TableObserver( new Handler() ) );
//		mResolver.registerContentObserver( DAContentProvider.PATIENTS_URI,
//				true, new TableObserver( new Handler() ) );
		mResolver.registerContentObserver( DAContentProvider.EXERCISE_ENTRIES_URI,
				true, new TableObserver( new Handler() ) );
		mResolver.registerContentObserver( DAContentProvider.GLUCOSE_ENTRIES_URI,
				true, new TableObserver( new Handler() ) );
		mResolver.registerContentObserver( DAContentProvider.MEAL_ENTRIES_URI,
				true, new TableObserver( new Handler() ) );
	}
	...
	public class TableObserver extends ContentObserver
	{
		...
		public void onChange( boolean selfChange, Uri changeUri )
		{
			...
			ContentResolver.requestSync( mAccount, DAContentProvider.AUTHORITY, settingsBundle );
